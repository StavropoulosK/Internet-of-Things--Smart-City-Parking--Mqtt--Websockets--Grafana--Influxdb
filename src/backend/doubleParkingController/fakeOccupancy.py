# einai o kodikas tis prosomiosis. Stin metabliti spots_To_Update bazoume ta id ton aisthitiron pou theloume na gemisoun oi thesis gia na diksoume oti litourgi to diploparkarisma.

spots_To_Update= [100461,100474]        # tha alaksoun oi aisthitires me id apo 100475 mexri 100482

fill=True    # An einai true oi thesis gemizoun, alios adiazoun
...
simulation_update_time_in_minutes = 2/60
...
def simulate():
    ...
    while True:
        ...
        for sensor in sensors:
            ...
            if(sensor.id >= min(spots_To_Update) and sensor.id <= max(spots_To_Update)):

                message = generateMessage(
                sensor.id,
                sensor.voltage,
                1.0 if fill else 0.0,
                tag,
                sensor.temperature,
                sensor.location[0],
                sensor.location[1],
                ...
                print('aa ',sensor.id,1.0 if fill else 0.)
